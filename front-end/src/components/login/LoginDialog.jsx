...
const LoginDialog = ({ open, setOpen }) => {
  ...
  const signupUser = async () => {
    let response = await authenticateSignup(signup, dispatch);
    if (!response) return;
    handleClose();
  ...
  const loginUser = async () => {
    let response = await authenticateLogin(login, dispatch);
        ...
        <Box style={{ display: "flex", height: "100%" }}>
          ...
          {account.view === "login" ? (
            <Wrapper>
              ...
              />
              <Text>
                By continuing , you agree to E-commerce Terms of Use and Privacy
                Policy.
              </Text>
